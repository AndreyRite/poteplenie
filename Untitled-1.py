...
# Команды о том, как предотвратить глобальное потепление

@bot.command(name="предотвратить_1")
async def prevent_1(ctx):
    await ctx.send("Сократите потребление энергии: выключайте свет и приборы, когда они не нужны, и используйте энергоэффективные лампочки.")

@bot.command(name="предотвратить_2")
async def prevent_2(ctx):
    await ctx.send("Сократите использование транспорта, особенно автомобилей на ископаемом топливе. Используйте общественный транспорт, велосипеды или ходите пешком.")

@bot.command(name="предотвратить_3")
async def prevent_3(ctx):
    await ctx.send("Сократите потребление мяса и других продуктов животного происхождения. Производство мяса вызывает значительные выбросы парниковых газов.")

@bot.command(name="предотвратить_4")
async def prevent_4(ctx):
    await ctx.send("Перерабатывайте отходы и уменьшайте их количество. Меньше отходов — меньше выбросов на свалках и в процессах производства.")

@bot.command(name="предотвратить_5")
async def prevent_5(ctx):
    await ctx.send("Поддерживайте инициативы и организации, направленные на борьбу с изменением климата, а также распространяйте информацию об этом среди других.")
